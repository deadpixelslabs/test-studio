...
Return exactly 6 layers with exactly 4 VISIBLE traits per layer. Include one required background and one required base layer. The remaining 4 layers must be optional (required=false) with noneWeight between 10 and 30 so absence is handled by the generator. The base layer must visibly define the user's main requested subject. Trait names must be specific to the requested concept. Base-layer traits must be genuinely different visible variants of the requested subject (fur/color/material/expression/body variant as appropriate); NEVER use numbered placeholders like 'Doge Punk 1', 'Variant 2', or generic filler names. Every trait must be visually meaningful and directly related to the prompt. IMPORTANT: NEVER use None, No Accessory, No Outfit, Empty, Transparent, or any absence/no-trait value inside the 4 trait names. Absence is represented ONLY by required=false plus noneWeight > 0. Required core layers must always contain 4 visible drawable traits. Weights are integers 1-100. The architecture must support at least 10,000 combinations. artDirection must define visual language, palette, and mood. compositionGuide must define one consistent centered composition. Do not output SVG code in this step.`,
    },
...
export function normalizePlan(data) {
  ...
  if (requiredCount !== 2) throw new Error('Plan must contain exactly the required background and base layers.');
