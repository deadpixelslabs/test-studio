export const TEXT_MODEL = process.env.MISTRAL_TEXT_MODEL || 'mistral-large-2512';
export const TEXT_FALLBACK_MODELS = String(process.env.MISTRAL_TEXT_FALLBACK_MODELS || 'mistral-small-2603,ministral-14b-2512')
  .split(',')
  .map((value) => value.trim())
  .filter(Boolean);
export const TEXT_MODEL_CHAIN = Array.from(new Set([TEXT_MODEL, ...TEXT_FALLBACK_MODELS]));
export const IMAGE_MODEL = process.env.MISTRAL_IMAGE_MODEL || 'mistral-small-2603';
...
export function compactMistralError(status, raw) {
  try {
    ...
    if (/not available in your subscription tier|subscription tier/i.test(message)) return 'This Mistral model is not available in your subscription tier.';
    if (/model.*not found|not found.*model|unknown model/i.test(message)) return 'The configured Mistral model is unavailable for this API key.';
...
function mistralHttpError(response, raw) {
  const message = compactMistralError(response.status, raw);
  const error = new Error(message);
  error.status = response.status;
  if (/not available in your subscription tier|configured Mistral model is unavailable/i.test(message)) {
    error.code = 'MODEL_UNAVAILABLE';
  }
  if (/image generation is not enabled/i.test(message)) {
    error.code = 'IMAGE_TOOL_UNAVAILABLE';
  }
...
function isRetryableMistralError(error) {
  ...
  return status === 429 || status === 502 || status === 503 || error?.code === 'TIMEOUT' || error?.code === 'MODEL_UNAVAILABLE';
}
